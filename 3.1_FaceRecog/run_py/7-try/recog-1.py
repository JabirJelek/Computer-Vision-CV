...
class RealTimeProcessor:
    ...
    def setup_image_logging(self, csv_filename: str = None):
        ...
        try:
            ...
            print(f"   - Format: PNG annotated frames")
    ...
    def has_mask_violations(self, results: List[Dict]) -> bool:
        ...
        for result in results:
            if (result.get('mask_status') == 'no_mask' and 
                result.get('mask_confidence', 0) > 0.6 and  # Confidence threshold
                result.get('identity') is not None):  # Only recognized faces
                return True
    ...
    def save_annotated_frame(self, frame: np.ndarray, results: List[Dict], original_frame: np.ndarray = None):
        ...
        try:
            ...
            # Generate filename
            filename = f"violation_{filename_time}_{self.saved_image_count:04d}.png"
    ...
    def toggle_logging(self, filename: str = None):
        ...
        if not self.logging_enabled:
            ...
            print("   - Images: PNG frames for mask violations")
    ...
    def log_performance_data(self, results: List[Dict], display_frame: np.ndarray = None):
        """Enhanced logging with image capture for mask violations"""
        if not self.logging_enabled:
            return
        ...
        try:
            ...
            if (self.image_logging_enabled and display_frame is not None and 
                ...
                self.save_annotated_frame(display_frame, results)
            ...
            if log_entries:
                ...
                violations = len([r for r in results if r.get('mask_status') == 'no_mask'])
                
                log_msg = f"📊 Logged {len(log_entries)} face entries"
                if violations > 0:
                    log_msg += f" | 🚨 {violations} mask violations"
                print(log_msg)
