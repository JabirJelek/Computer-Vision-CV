...
class ByteTrackerThread:
    """
    Modular ByteTracker thread using Ultralytics YOLO's built-in tracking
    Receives frames from SelectiveFrameProcessor and outputs tracked objects
    """
    def __init__(self, model_config="yolo11n.pt", tracker_config="bytetrack.yaml", 
        ...
        self.processing_queue = queue.Queue(maxsize=50)  # Single queue for simplicity
    ...
    def stop(self):
        """Stop the tracking thread - FIXED VERSION"""
        self.running = False
        if self.thread:
    ...
    def process_frame(self, frame, frame_num):
        ...
        try:
            ...
            self.processing_queue.put(data, block=False)  # Use processing_queue
            return True
        except queue.Full:
    ...
    def get_results(self):
        ...
        try:
            return self.processing_queue.get(block=False)  # Use processing_queue
        except queue.Empty:
            return None

    def _tracking_loop(self):
        """Minimal tracking loop"""
        while self.running:
            try:
                # Get frame from queue
                data = self.processing_queue.get(timeout=0.5)  # Use processing_queue
                frame = data['frame']
                ...
                # Process results minimally
                tracked_data = self._process_results(results, frame, frame_num)
                
                if tracked_data:
                    # Replace the data with tracking results
                    data.update(tracked_data)
                
                self.processing_queue.task_done()  # Use processing_queue
    ...
    def _process_results(self, results, frame, frame_num):
        """Minimal results processing - FIXED VERSION"""
        ...
        # Return the frame along with tracking data
        return {
            'tracked_objects': tracked_objects,
            'frame': frame,  # Add the frame back
            'frame_num': frame_num,
...
class SelectiveFrameProcessor:
    ...
    def __init__(self, source=0, fps=30, processing_interval=0.5, is_rtsp=False, display_width=640, 
        ...
        # ByteTracker integration parameters - NOW PROPERLY DEFINED
        self.enable_tracking = enable_tracking
        self.tracker_config = tracker_config
        self.tracking_model = tracking_model
        
        if self.enable_tracking:
            # Initialize ByteTracker thread
            ...
            print("✅ ByteTracker integrated into pipeline")
        else:
