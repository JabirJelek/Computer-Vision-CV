...
class SavingDetectionManager:
    """
    Manages saving detected frames from alert classes with bounding boxes
    Now uses the same annotated frames as SelectiveFrameProcessor for consistency
    """
    def __init__(self, alert_classes=None, save_dir="detected_frames", server_url=None,
                 bbox_label_map=None):
        ...
        # Bounding box label mapping from custom_bbox configuration
        self.bbox_label_map = bbox_label_map or {}
        
        # Create save directory
        self.save_dir.mkdir(exist_ok=True)
        
        # Frame saving state management
        self.frame_count = 0
        
        # Frame saving and sending state
        self.last_save_time = {}
        self.min_save_interval = 2.0  # Minimum seconds between saves per class

        # Default color for unconfigured classes
        self.default_color = (0, 255, 0)  # Green

        print(f"📸 SavingDetectionManager initialized")
        print(f"📸 Alert classes for frame saving: {self.alert_classes}")
        if self.bbox_label_map:
            print(f"🎨 Using custom bbox colors from configuration")

    def process_detection(self, annotated_frame, detected_objects, frame_num, timestamp):
        """Process detection results and save frames for alert classes"""
        if not detected_objects:
            return False
            
        alert_detected = False
        alert_classes_in_frame = set()
        ...
        for obj in detected_objects:
            ...
            if class_id in self.alert_classes:
                alert_detected = True
                alert_classes_in_frame.add(class_id)
                
                # Handle alert with rate limiting
                if self._should_save_frame(class_id, timestamp):
                    self._handle_alert_frame(class_id, confidence)
        
        # Save frame if any alert class detected
        if alert_detected:
            self._save_detected_frame(annotated_frame, frame_num, timestamp, detected_objects, alert_classes_in_frame)
            return True
            
        return False

    def _should_save_frame(self, class_id, timestamp):
        """Check if we should save this frame (rate limiting per class)"""
        current_time = time.time()
        class_key = f"class_{class_id}"
        
        # Check if we should save this frame (rate limiting per class)
        if class_key in self.last_save_time:
            time_since_last_save = current_time - self.last_save_time[class_key]
            if time_since_last_save < self.min_save_interval:
                return False  # Skip saving, too soon
        
        # Update last save time
        self.last_save_time[class_key] = current_time
        return True

    def _handle_alert_frame(self, class_id, confidence):
        """Handle frame with detected alert class"""
        # Get display name for logging
        display_label = self._get_display_label(class_id)
        print(f"📸 Alert class {display_label} detected (Conf: {confidence:.2f})")

    def _get_display_label(self, class_id):
        """Get display label for a class based on custom bbox configuration"""
        ...
        # Default behavior - use class ID
        return f"Class_{class_id}"

    def _save_detected_frame(self, annotated_frame, frame_num, timestamp, detected_objects, alert_classes_in_frame):
        """Save detected frame as PNG (uses the same annotated frame as display)"""
        try:
            # Create filename with timestamp and frame number
            timestamp_str = time.strftime("%Y%m%d_%H%M%S")
            
            # Include alert classes in filename for easy identification
            class_names = []
            for class_id in alert_classes_in_frame:
                display_label = self._get_display_label(class_id)
                class_names.append(display_label.replace(" ", "_"))
            
            classes_str = "_".join(class_names)
            filename = f"alert_{timestamp_str}_frame{frame_num}_{classes_str}.png"
            filepath = self.save_dir / filename
            
            # Save the annotated frame (already has bounding boxes drawn by main processor)
            ...
            if success:
                ...
                if self.server_url:
                    self._send_to_server(filepath, detected_objects, frame_num)
            else:
    ...
    def _send_to_server(self, filepath, detected_objects, frame_num):
    ...
    def set_bbox_label_map(self, bbox_label_map):
        """Update bounding box label mapping"""
        self.bbox_label_map = bbox_label_map
        print(f"🎨 Updated bbox label map with {len(bbox_label_map)} entries")
...
class SelectiveFrameProcessor:
    ...
    Now with integrated SavingDetectionManager for alert frame saving using same annotated frames
    """
    ...
    def __init__(self, source=0, fps=30, processing_interval=0.5, is_rtsp=False, display_width=640, 
        ...
        if self.is_rtsp:
            ...
            self.capture.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Reduce buffer size
        ...
        # SavingDetectionManager integration
        if self.enable_frame_saving:
            # Initialize SavingDetectionManager with alert classes
            self.saving_detector = SavingDetectionManager(
                alert_classes=list(self.alert_classes.keys()) if self.alert_classes else [],
                save_dir=frame_save_dir,
                server_url=frame_server_url,
                bbox_label_map=self.bbox_label_map
            )
            print("✅ SavingDetectionManager integrated with alert frame saving")
            if frame_server_url:
    ...
    def _run_yolo_detection(self, frame, frame_num):
        ...
        try:
            ...
            # Extract detection data for saving
            detected_objects = []
            if results and len(results) > 0 and results[0].boxes:
            ...
            if results and len(results) > 0:
                ...
                return annotated_frame, detections_count, detected_objects
            
            return frame, 0, detected_objects
    ...
    def _add_info_overlay(self, frame, frame_num, processed_count, detections_count):
        ...
        cv2.putText(frame, f"Source: {source_type}", (10, 25), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 255), thickness)
        cv2.putText(frame, f"Frame: {frame_num}", (10, 45), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), thickness)
        cv2.putText(frame, f"Processed: {processed_count}", (10, 65), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 0), thickness)
        cv2.putText(frame, f"Detections: {detections_count}", (10, 85), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 0, 255), thickness)
        cv2.putText(frame, f"Total Detections: {self.detection_count}", (10, 105), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 0, 255), thickness)
        
        # Add alert status
        alert_status = f"Active Alerts: {len(self.active_alerts)}"
        alert_color = (0, 0, 255) if self.active_alerts else (0, 255, 0)
        cv2.putText(frame, alert_status, (10, 125), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale, alert_color, thickness)
        ...
        if self.active_alerts:
            ...
            cv2.putText(frame, alert_text, (10, 145), 
                      cv2.FONT_HERSHEY_SIMPLEX, font_scale-0.1, (0, 0, 255), 1)
        
        cv2.putText(frame, f"Time: {timestamp}", (10, 160), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale-0.1, (255, 255, 255), 1)
        cv2.putText(frame, f"Interval: {self.processing_interval}s", (10, 175), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale-0.1, (255, 255, 255), 1)
        cv2.putText(frame, "Press ESC to exit", (10, 190), 
                  cv2.FONT_HERSHEY_SIMPLEX, font_scale-0.1, (255, 255, 255), 1)
    ...
    def _processing_loop(self):
        """Optimized processing loop with SavingDetectionManager"""
        print("Processing thread started with SavingDetectionManager")
        frames_processed = 0
        ...
        while self.running:
            ...
            if elapsed >= self.processing_interval:
                ...
                if frame_to_process is not None:
                    ...
                    # Run YOLO detection and get annotated frame with detection data
                    annotated_frame, detections_count, detected_objects = self._run_yolo_detection(frame_to_process, frame_num)
                    
                    # Process frame saving with SavingDetectionManager
                    if self.enable_frame_saving and self.saving_detector and detected_objects:
                        self.saving_detector.process_detection(
                    ...
                    if annotated_frame is not None:
                        resized_frame = self._resize_frame(annotated_frame)
                        self._add_info_overlay(resized_frame, frame_num, frames_processed, detections_count)
    ...
    def reload_alert_classes(self, new_alert_classes_path=None):
        ...
        # Update SavingDetectionManager with new alert classes
        if self.enable_frame_saving and self.saving_detector:
            self.saving_detector.set_alert_classes(list(self.alert_classes.keys()))
        ...
        for box in boxes:
            ...
            if should_display:
                ...
                if confidence > 0.8:
                    ...
                    marker_length = 15
                    # Top-left corner
                    cv2.line(annotated_frame, (x1, y1), (x1 + marker_length, y1), box_color, 3)
                    cv2.line(annotated_frame, (x1, y1), (x1, y1 + marker_length), box_color, 3)
                    # Top-right corner  
                    cv2.line(annotated_frame, (x2, y1), (x2 - marker_length, y1), box_color, 3)
                    cv2.line(annotated_frame, (x2, y1), (x2, y1 + marker_length), box_color, 3)
                    # Bottom-left corner
                    cv2.line(annotated_frame, (x1, y2), (x1 + marker_length, y2), box_color, 3)
                    cv2.line(annotated_frame, (x1, y2), (x1, y2 - marker_length), box_color, 3)
                    # Bottom-right corner
                    cv2.line(annotated_frame, (x2, y2), (x2 - marker_length, y2), box_color, 3)
                    cv2.line(annotated_frame, (x2, y2), (x2, y2 - marker_length), box_color, 3)
    ...
    def reload_bbox_labels(self, new_config_path=None):
        ...
        # Update SavingDetectionManager with new bbox label map
        if self.enable_frame_saving and self.saving_detector:
            self.saving_detector.set_bbox_label_map(self.bbox_label_map)
