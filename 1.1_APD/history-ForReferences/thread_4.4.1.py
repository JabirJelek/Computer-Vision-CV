...
class AudioAlertManager:
    ...
    def stop(self):
        ...
        print("Initiating enhanced shutdown sequence...")
        self.running = False
        
        # Stop audio alerts first
        if self.audio_alert_manager:
            print("Stopping audio alert manager...")
            self.audio_alert_manager.stop()
        
        # Proper thread termination with timeouts
        threads = []
        if self.capture_thread and self.capture_thread.is_alive():
            threads.append(("Capture", self.capture_thread))
        if self.processing_thread and self.processing_thread.is_alive():
            threads.append(("Processing", self.processing_thread))
        ...
        print(f"Shutdown complete. Total detections: {self.detection_count}")
...
class SelectiveFrameProcessor:
    ...
    def _processing_loop(self):
        """Stabilized processing loop - FIXES RACE CONDITION"""
            ...
            if elapsed >= self.processing_interval:
                ...
                with self.lock:
                    if self.latest_frame is not None:
                        ...
                        # CRITICAL FIX: Don't clear latest_frame - let capture thread manage it
