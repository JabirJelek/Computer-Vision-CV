...
class ObjectDetector:
    ...
    def process_frame(self, frame):
        ...
        # Your sequential logic implementation
        current_frame_classes = set()
        sequence_detections = []  # Store detections for sequential processing
        ...
        for r in results:
            ...
            if r.boxes is not None and len(r.boxes) > 0:
                for i, box in enumerate(r.boxes):
                    ...
                    if cls_id in self.config.special_classes:
                        ...
                        sequence_detections.append((box, mask, cls_id, conf, class_name))
            ...
            self._apply_sequential_logic(frame, current_frame_classes, sequence_detections)        
    ...
    def _apply_sequential_logic(self, frame, current_frame_classes, sequence_detections):
            """Apply your sequential detection logic"""
            required_classes = set(self.config.special_classes)
            
            # Check if we have Class 3 in current frame
            class3_id = 2  # Assuming class 3 is ID 2
            if class3_id in current_frame_classes:
                # Check if Class 1 and 2 are in memory but not in current frame
                ...
                class1_in_current = 0 in current_frame_classes
                class2_in_current = 1 in current_frame_classes
                
                # YOUR CONDITION: Class 3 present, but Class 1/2 only in memory
                if (class1_in_memory or class1_in_current) and (class2_in_memory or class2_in_current):
                    # All three classes are present in the sequence (current frame + memory)
                    special_boxes = [box for box, _, _, _, _ in sequence_detections]
                    special_masks = [mask for _, mask, _, _, _ in sequence_detections]
