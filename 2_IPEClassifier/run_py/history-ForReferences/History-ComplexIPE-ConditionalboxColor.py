...
class AppConfig:
    ...
    # Model configuration
    model_path: Path = Path(r"C:\Farid\Dokumen\Object Detection\CV_model\bestIPE-2.torchscript")
    ...
    conditional_messages: Dict[str, str] = None  # Messages for specific combinations
    ...
    def __post_init__(self):
        ...
        if self.conditional_box_colors is None:
            self.conditional_box_colors = {
                "all_items": (0, 255, 255),  # Yellow for all three classes
                "0_and_1": (255, 0, 0),      # Blue for classes 0 and 1
                "0_and_2": (0, 0, 255),      # Red for classes 0 and 2
                "1_and_2": (255, 0, 255)     # Magenta for classes 1 and 2
            }
            
        if self.conditional_messages is None:
            self.conditional_messages = {
                "0_and_1": "Missing Fork",
                "0_and_2": "Missing Menu",
                "1_and_2": "Missing Rice",
                "all_items": "All Items Present"
            }
...
class DetectionVisualizer:
    ...
    def draw_detection(self, frame, box, class_name, conf, cls_id, is_conditional=False, conditional_type=None):
        ...
        if is_conditional:
            if conditional_type in self.config.conditional_box_colors:
                color = self.config.conditional_box_colors[conditional_type]
            else:
    ...
    def draw_conditional_bounding_box(self, frame, boxes, conditional_type):
        ...
        # Get color and message based on conditional type
        color = self.config.conditional_box_colors.get(conditional_type, (255, 255, 255))
        message = self.config.conditional_messages.get(conditional_type, "Condition Met")
        ...
        (text_width, text_height), _ = cv2.getTextSize(
            message, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 3
        )
...
class ObjectDetector:
    ...
    def process_frame(self, frame):
        ...
        # Check for specific conditional cases
        if special_class_ids == set(self.config.special_classes):
            ...
            self.visualizer.draw_conditional_bounding_box(frame, special_boxes, "all_items")
        elif special_class_ids == {0, 1}:
            # Classes 0 and 1 detected
            special_boxes = [box for box, _, _, _ in special_detections]
            self.visualizer.draw_conditional_bounding_box(frame, special_boxes, "0_and_1")
        elif special_class_ids == {0, 2}:
            # Classes 0 and 2 detected
            special_boxes = [box for box, _, _, _ in special_detections]
            self.visualizer.draw_conditional_bounding_box(frame, special_boxes, "0_and_2")
        elif special_class_ids == {1, 2}:
            # Classes 1 and 2 detected
            special_boxes = [box for box, _, _, _ in special_detections]
            self.visualizer.draw_conditional_bounding_box(frame, special_boxes, "1_and_2")
        # If only 1 special class is detected, do nothing (don't display it)
...
def main():
    ...
    config = AppConfig(
        model_path=Path(r"C:\Farid\Dokumen\Object Detection\CV_model\bestIPE-2.torchscript"),
        ...
        conditional_messages={
            "0_and_1": "Missing Menu",
            "0_and_2": "Missing Rice",
            "1_and_2": "Missing Fork",
            "all_items": "All Items Present"
        }
    )
